...
global.onChangeSearchInput = function (event) {
    ...
    if (query.length > 0){
        ...
        .then(function(data) {
            ...
            if (nOptions > 0) {
                selector.size = Math.max(nOptions, 2); // when only 1 option set the size to 2 to avoid collapsing
                for (var i = 0; i < nOptions; i++) {
                    var option = document.createElement("option");
                    option.value = cities[i].city + "," + cities[i].state + "," + cities[i].country;
                    option.text = cities[i].city + " (" + cities[i].state + "), " + cities[i].country;
